...
contract DirectDepositorPausable_Integration_Concrete_Test is MachinePeriphery_Integration_Concrete_Test {
    DirectDepositorPausable internal directDepositor;

    function setUp() public virtual override {
        MachinePeriphery_Integration_Concrete_Test.setUp();

        vm.prank(dao);
        directDepositor = DirectDepositorPausable(
            hubPeripheryFactory.createDepositor(
                DIRECT_DEPOSITOR_PAUSABLE_IMPLEM_ID, abi.encode(DEFAULT_INITIAL_WHITELIST_STATUS)
            )
        );
